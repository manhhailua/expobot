...
const RetrieveFineTunePage = () => {
  ...
  const renderTitle = () => {
    return (
      <Stack direction="row" spacing={1}>
        <Typography variant="h3" sx={{ m: "auto 0" }}>{fineTune.fine_tuned_model}</Typography>
        <IconButton
          onClick={() => copy(fineTune.fine_tuned_model)}
          size="small"
          color="primary"
        >
          <IconCopy />
  ...
  return (
    <>
      <MainCard
        title={renderTitle()}
        secondary={<Chip label={fineTune.model} variant="outlined" />}
      >
